...
export const DesktopActionBar = ({
  ...
  <div
    ...
    className={cn(
      "flex min-h-10 items-center justify-between gap-3 border-t border-border-subtle bg-surface px-4 py-2",
      className,
    )}
  >
    <div className="min-w-0 flex-1 text-xs text-muted-foreground">{left}</div>
...
export const DesktopSplitPaneReview = ({
  ...
  <div
    ...
    className={cn(
      "grid h-full min-h-[34rem] grid-cols-[var(--erp-shell-split-pane-min-width)_minmax(0,1fr)] overflow-hidden rounded-card border border-border bg-surface shadow-card",
      review && "xl:grid-cols-[var(--erp-shell-split-pane-min-width)_minmax(0,1fr)_20rem]",
      className,
    )}
  >
    <section className="min-h-0 border-r border-border-subtle">
    ...
    {review && (
      <section className="hidden min-h-0 border-l border-border-subtle bg-surface-muted xl:block">
...
const PaneHeader = ({ children }: { children: ReactNode }) => (
  <div className="flex h-10 items-center border-b border-border-subtle bg-surface-muted px-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
    {children}
  </div>
);
